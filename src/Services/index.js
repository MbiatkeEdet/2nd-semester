...
const createRepository = async (data) => {
  return await axios.post("https://api.github.com/user/repos", data, {
    headers: {
      Authorization: `Bearer ${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}`,
...
const updateRepository = async (name, data) => {
  return await axios.patch(
    `https://api.github.com/repos/${MbiatkeEdet}/${mbiatke}`,
    data,
    {
      headers: {
        Authorization: `Bearer ${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}`,
...
const allRepositories = async (page = 1, perPage = 10) => {
  return await axios.get(`https://api.github.com/users/${MbiatkeEdet}/repos`, {
    headers: {
      Authorization: `Bearer ${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}`,
      Accept: "application/vnd.github.v3+json",
    },
...
const singleRepository = async (id) => {
  return await axios.get(`https://api.github.com/repositories/${id}`, {
    headers: {
      Authorization: `Bearer ${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}`, // Add a comma here
      Accept: "application/vnd.github.v3+json",
    },
  });
};
